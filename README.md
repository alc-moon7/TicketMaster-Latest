...
- Watchlist matches `IMG_0370.PNG`, with working Events/Favourites segmented selection, responsive empty state and benefit rows. Its dark background extends behind the phone status bar so network and notification icons remain visible. The small star beside the Home tab logo appears only when Home is selected. Watchlist copy remains long-press editable; it does not yet persist actual watched events.
...
- My Events restores the same saved ticket entries, or creates one default Colorado Rockies vs. San Diego Padres card at Coors Field, dated Sep 14, 2026. All prototype tickets stay Upcoming regardless of their editable date or the phone date; Past stays empty. Saved dates, IDs and ticket data are unchanged. An empty saved list is treated as absence of saved tickets.
...
- Double-tap/long-press a ticket card to show camera/gallery image options. Choosing an image applies it immediately without opening a crop screen. A single tap opens details.
...
- The V2 ticket UI follows `V2/Screenshot_20260918-201930.png` (list), `201357`/`201419` (expanded event), `201401`/`201422` (collapsed event), `201440` (Extras), the generated white barcode/information references, then `200612`/`200616` (selection), `200417` (recipient method), and `200425`/`200433`/`200454` (recipient forms). Existing ticket content is retained rather than replacing saved data with screenshot examples. The event-details Tickets tab lays every ticket card out vertically instead of paging horizontally; all cards share the first ticket's persisted Section and Row values, so editing either field on any card updates the full set.
- Home keeps the location header visible while scrolling, with its chevron immediately after the location name. Its dark, draggable Change Location sheet opens at full height like the reference, uses the reference's compact search, heading, flag and location-row scale, shows New York under Recent Locations, and keeps the previous popular/search locations while adding Seattle. The cyan near-me button uses the reference's angled navigation arrow. Typing at least two letters shows locally matched city, region, or country suggestions directly below the search box, with city-name prefix matches first; selecting a result saves the chosen header label with the existing `discover-location` text key. The near-me icon shows a location-services prompt; device geolocation is not integrated. The Home feed follows the observed For You, Trending, event, presale, category, promoted and City Guides sections in order. Each carousel scrolls sideways; View All opens a separate dark scrollable list with back navigation. Captured reference artwork and sample entries are bundled under `assets/reference/home/`; this is not a live event feed. Previous Home heading, trending and first three card/city text keys remain attached to the corresponding new positions, so saved long-press edits remain available. My Tickets uses a charcoal header, purple event panels, 16:9 dynamic artwork, blue dividers and View Tickets footers. The list and event-details header use scoped Source Sans 3 fonts from the Android reference APK; Extras uses scoped Averta. The date-derived next-event label is presentation only; Upcoming/Past tab switching, count shortcuts, image options, quantity badge, search and editable text keys remain intact. The quantity badge text is native Material text so its long-press changes quantity instead of opening text editing. Empty/search states remain readable on dark backgrounds.
- Ticket details follows the Z72 reference's white content layout: an inset event summary below the purple artwork, a collapsed title/venue/barcode header, pinned Tickets/Extras tabs, an order-details sheet, seat PageView and dots, metadata link, 16:9 map and floating Transfer/Sell actions (hidden on Extras). Switching to Extras preserves the current outer scroll position, so its heading begins below the tabs instead of being skipped by an automatic scroll. The pinned scanner also opens View Ticket when the hero has scrolled away. View Order and the preserved Ticket Details link open the existing information route. Receipt, Sell and Get Directions remain presentation buttons. Transfer opens the ticket-selection sheet directly over event details with the first ticket selected; its section, row and seat labels use editable ticket values. The old Tickets/Event Info/Venue Info transfer landing screen and route were removed. The sample order/seat values are not live order data.
- View Ticket opens on the first ticket (`1 of N`, using the card's ticket quantity), retains paging arrows and the animated blue line over the existing sample barcode, and uses a scrollable white ticket, purple artwork, wallet and metadata controls. Seat defaults increase by ticket position (1, 2, 3, etc.) in the details card and View Ticket; saved per-ticket seat edits still take precedence. Wallet remains a placeholder. Ticket information has scrollable Ticket Details/Event Information tabs. Existing metadata is preserved.
- Transfer selection/continue/back remain unchanged. The recipient-method sheet opens a local manual form with name, email/mobile toggle, clear controls and optional note. The mobile field uses the normal text keyboard and shows the `(XXX) XXX-XXXX` placeholder because the US `+1` prefix is already displayed outside the field. The Transfer button enables in black with white text only when both names contain letters and the selected email or US +1 mobile number has valid syntax; the note is optional. A normal tap does not open the error; holding the Transfer button shows the reference technical-difficulties dialog with the selected ticket count and updates Account → My Inbox with one local confirmation-style mail for the latest attempted transfer. The ticket-count number inside that dialog supports the persisted long-press text editor. The confirmation mail fills the available inbox width and height below its app bar, with no outer card margin; Stay Connected sits at the bottom and remains reachable by scrolling on shorter screens. That mail uses the selected ticket's saved title/date/venue, image and each selected section/row/seat; a later attempt replaces it. Mail metadata is cached with the existing per-user edited-text state, and the image resolves from the ticket's saved selection. Every visible mail text label uses the existing long-press editor with a key scoped to that transfer attempt, so edits do not carry into the next mail. Long-press or double-tap the mail's event image to choose Gallery or Camera; the selected full image is applied immediately without a crop screen and updates the saved ticket image too. This is in-app sample UI: fields are not sent to a transfer service, contacts remains a placeholder, and no actual tickets or email are sent. Sheets scroll on short phones and expand immediately for the keyboard, with a reachable back footer.
...
Ticket images store the original selected bytes plus original dimensions and a rectangle normalized to 0..1; they do not create a cropped file. My Tickets and the inbox mail image editor both apply a newly selected camera/gallery image immediately with a full-image `0,0,1,1` rectangle, without opening the crop screen. TicketCardImageViewport scales/translates the original image within ClipRect. Preserve normalized geometry during serialization changes.
...
- Android applicationId/namespace: `com.ticketmaster.mobile.android`; minSdk follows Flutter (currently 24); Java/Kotlin JVM target 17; NDK 29.0.14206865; AGP 8.11.1; Kotlin plugin 2.2.20; Gradle 8.14; Google Services plugin 4.3.10. Compile/target SDK follow Flutter. Gradle heap/metaspace are capped at 3 GB/1 GB to fit this 8 GB build machine. Release and debug both sign with `android/app/ticketmaster-signing.jks` (alias `ticketmaster`, store/key password read from `android/key.properties`).
- Android release builds check the public GitHub repository's latest Release API before resolving login or splash; debug/profile builds skip the check so `flutter run` and hot restarts do not consume the anonymous API quota. A release tag in exact `vMAJOR.MINOR.PATCH+BUILD` form (for example `v2.0.1+3`) and at least one `.apk` Release asset block app startup when BUILD exceeds the installed build. The GitHub Release description becomes What's New; any legacy `[force-update]` marker is hidden. The update dialog has no Later/back dismissal. Update first requests Android's one-time unknown-app-source grant when needed, then downloads the APK into the app cache with progress and opens the Android package installer through FileProvider; Android still requires the user to confirm installation. Canceling installation leaves the gate in place and allows retry. Each APK must use the same signing key and an increased pubspec build number. A network failure during the update check no longer blocks startup; the app retries every 15 seconds while open and shows the update dialog if a newer APK becomes reachable. Non-network check errors still show the Retry dialog. The releases repo must be public; never embed a private-repo token in the app.
...
- V2 validation on 2026-09-20: `flutter test --no-pub` passes all 9 tests. Firebase host calls are mocked; tests do not use live accounts or write user data. Coverage includes default tickets shown immediately in Upcoming with Past empty and tab switching back, current bottom navigation, list/detail/barcode/information routes at 320×568, 360×640, 412×915, 430×932 and 640×360; hidden card-count/quantity/image gestures, text-edit dialog, search, transfer selection/back, manual email/mobile form, keyboard insets and 1.5× list text.
...
- The first V2 pass compiled via hot reload and was visually checked on the connected Android phone; subsequent refinements were compiled and checked with widget tests and rendered snapshots. Release APK builds successfully on 2026-09-19 with `flutter build apk --release --no-pub` after reducing oversized Gradle memory caps; Android `apksigner verify` confirms its v2 signature, and `aapt` confirms package `com.ticketmaster.mobile.android`, version 2.0.1+3. Native camera/gallery, live cloud sync, installed release behavior and iOS builds were not revalidated. Persistence, IDs and normalized crop serialization were not changed.
