...
Ticket images store the original selected bytes plus original dimensions and a rectangle normalized to 0..1; they do not create a cropped file. My Tickets applies a newly selected camera/gallery image immediately with a full-image `0,0,1,1` rectangle, without opening the crop screen. The inbox mail image editor still supports an aspect-locked crop. TicketCardImageViewport scales/translates the original image within ClipRect. Preserve normalized geometry during serialization changes.
