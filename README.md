...
- Android release builds resolve GitHub's public `releases/latest/download/app-release.apk` redirect before login or splash, avoiding the anonymous REST API rate limit; debug/profile builds skip the check. Publish every release with the exact asset name `app-release.apk` and a tag in `vMAJOR.MINOR.PATCH+BUILD` form (for example `v2.0.1+3`). A higher BUILD blocks startup with a non-dismissible update dialog. Update first requests Android's one-time unknown-app-source grant when needed, then downloads the APK into the app cache and opens the Android package installer; Android still requires confirmation. Canceling installation leaves the gate in place and allows retry. Each APK must use the same signing key and an increased pubspec build number. Network failures do not block startup; the app retries every 15 seconds while open. Other malformed release/check errors show the Retry dialog. The releases repo must remain public; never embed a private-repo token in the app.
